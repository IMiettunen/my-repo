...
#Tämä versio selvittää Criss Cross tason paremmin, 
#mutta ei läpäise Don't crash tasoa
...
def angleDiff(ac_dir, ap_dir):
    ...
    """
    if abs(ac_dir - ap_dir) > 180:
        return 360 - abs(ac_dir - ap_dir)
    else:
        return abs(ac_dir - ap_dir)
...
def approachPoint(ac_dir, ap_dir, ap_x, ap_y, ext = False):
    ...
    #Minimum radius that is enough to make the turn to airport rounded up to five because speed is 5 per tick
    #Default is 10
    if ext:
...
def getNextPoint(aircraft, direction):
    ...
    """
    ticksAhead = 5
    #nextX = aircraft['position']['x'] + speed*int(aircraft['speed']) * math.cos(math.radians(aircraft['direction']))
    #nextY = aircraft['position']['y'] + speed*int(aircraft['speed']) * math.sin(math.radians(aircraft['direction']))
...
def collision(aircrafts, ac1, direction):
    ...
    for ac2 in aircrafts:
        if ac2['id'] != ac1['id']:
            ...
            if distanceBetween(ac1NextX, ac1NextY, ac2NextX, ac2NextY) <= ac1['collisionRadius']+10:
                return True           
...
##################
        ...
        if len(game_state["aircrafts"]) > 1 and collision(game_state["aircrafts"], aircraft, ac_dir):
            new_dir = normalize_heading(ac_dir-20)
